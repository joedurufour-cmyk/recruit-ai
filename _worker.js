...
<body>
    ...
    <div class="container">
        ...
        <div class="settings-panel" id="settingsPanel">
            ...
            <div class="settings-grid">
                <div class="form-group">
                    ...
                    <input type="url" id="backendUrl" placeholder="https://recruit-ai-api.onrender.com">
                </div>
                <div class="form-group">
                    <label>API Secret</label>
                    <div style="display:flex; gap:0.5rem;">
                        <input type="password" id="apiSecret" placeholder="your-secret-key" style="flex:1;">
                        <button class="btn btn-secondary btn-sm" onclick="toggleSecretVisibility()">👁</button>
                    </div>
                </div>
            </div>
            <div style="display:flex; gap:0.5rem; margin-top:1rem;">
                <button class="btn btn-primary btn-sm" onclick="saveSettings()">Guardar</button>
                <button class="btn btn-secondary btn-sm" onclick="testConnection()">Probar Conexión</button>
            </div>
            <p style="margin-top:0.75rem; font-size:0.8rem; color:var(--text-secondary);">
                💡 El backend oculta tu API key de Kimi. Los usuarios finales nunca la ven.
            </p>
        </div>
        ...
        <div id="cv" class="module active">
            <div class="card">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Industria</label>
                        <select id="cvIndustry">
                            <option value="general">General</option>
                            <option value="tech">Tecnología / Software</option>
                            <option value="finance">Finanzas / Banking</option>
                            <option value="marketing">Marketing / Publicidad</option>
                            <option value="sales">Ventas / Business Dev</option>
                            <option value="design">Diseño / UX/UI</option>
                            <option value="operations">Operaciones / Logística</option>
                            <option value="hr">RRHH / Talento</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label>Pega tu CV aquí (texto plano)</label>
                    <textarea id="cvContent" placeholder="Pega todo el contenido de tu CV..." rows="8"></textarea>
                </div>
                <button class="btn btn-primary" id="cvBtn" onclick="analyzeCv()">Analizar con IA</button>
                <div id="cvResult" class="result-container"></div>
                <div id="cvHistory" style="margin-top:1rem;"></div>
            </div>
        </div>
        ...
        <div id="job" class="module">
            <div class="card">
                ...
                <div class="form-grid">
                    <div class="form-group"><label>¿Qué tan interesado estás?</label>
                        <select id="jobInterest">
                            <option value="low">Curiosidad</option>
                            <option value="medium" selected>Medianamente</option>
                            <option value="high">Lo quiero</option>
                        </select>
                    </div>
                </div>
        ...
        <div id="freelance" class="module">
            <div class="card">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Tarifa esperada (USD/hora)</label>
                        <select id="flRate">
        ...
        <div id="interview" class="module">
            <div class="card">
                ...
                <div class="form-group"><label>Tu mayor desafío en entrevistas</label>
                    <select id="intChallenge">
                        <option value="">General</option>
                        <option value="nervousness">Nerviosismo / Confianza</option>
                        <option value="technical">Preguntas técnicas</option>
                        <option value="behavioral">Behavioral / STAR</option>
                        <option value="salary">Negociación salarial</option>
                        <option value="explanation">Explicar decisiones</option>
                        <option value="questions">Qué preguntar</option>
                    </select>
                </div>
    ...
    <script>
        // ========== CONFIG ==========
        const CONFIG = {
            backendUrl: localStorage.getItem('ra_backendUrl') || '',
            apiSecret: localStorage.getItem('ra_apiSecret') || ''
        };
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            updateStatus(CONFIG.backendUrl && CONFIG.apiSecret ? 'warning' : 'danger',
                CONFIG.backendUrl && CONFIG.apiSecret ? '⚠️ Falta probar conexión' : '⚙️ Configurar API');
        ...
        async function testConnection() {
            ...
            try {
                ...
                if (r.ok) {
                    updateStatus('connected', '✓ Backend conectado');
                } else {
                    updateStatus('danger', \`✗ Error \${r.status}: Revisa API Secret\`);
                }
            } catch(e) {
                updateStatus('danger', '✗ Backend no responde. Revisa URL.');
            }
        }
        ...
        async function callApi(endpoint, body, btnId, resultId) {
            if (!CONFIG.backendUrl || !CONFIG.apiSecret) {
                alert('⚠️ Configura Backend URL y API Secret primero (click en el status arriba)');
                toggleSettings();
                return null;
            }
            ...
            try {
                const r = await fetch(\`\${CONFIG.backendUrl}\${endpoint}\`, {
                    method: 'POST',
                    headers: {
                ...
                saveToHistory(endpoint, body);
                return data;
            } catch(e) {
                showResult(resultId, '❌ Error', e.message, 'error');
                return null;
            } finally {
                btn.disabled = false;
        ...
        function showResult(id, title, html, type) {
            const el = document.getElementById(id);
            el.className = \`result-container show \${type === 'error' ? 'error' : type === 'warning' ? 'warning' : ''}\`;
            el.innerHTML = \`<div class="result-title">\${title}</div><div class="result-content">\${html}</div>\`;
        }

        // ========== MODULE ACTIONS ==========
        async function analyzeCv() {
            const content = document.getElementById('cvContent').value.trim();
            if (!content) { alert('⚠️ Pega tu CV'); return; }
            await callApi('/api/cv/optimize', {
        ...
        async function analyzeLinkedin() {
            ...
            const bio = document.getElementById('liBio').value.trim();
            if (!title || !bio) { alert('⚠️ Completa título y bio'); return; }
            await callApi('/api/linkedin/analyze', {
                title: title,
                bio: bio,
        ...
        async function analyzeJob() {
            ...
            const p = document.getElementById('jobProfile').value.trim();
            if (!t || !d || !p) { alert('⚠️ Completa todos los campos'); return; }
            await callApi('/api/job/match', {
                job_title: t,
                job_description: d,
                user_profile: p,
        ...
        async function searchFreelance() {
            const s = document.getElementById('flSpecialty').value;
            const sk = document.getElementById('flSkills').value.trim();
            if (!s || !sk) { alert('⚠️ Selecciona especialidad y skills'); return; }
            await callApi('/api/freelance/search', {
                specialty: s,
        ...
        async function prepInterview() {
            const pos = document.getElementById('intPosition').value.trim();
            const prof = document.getElementById('intProfile').value.trim();
            if (!pos || !prof) { alert('⚠️ Completa puesto y perfil'); return; }
            await callApi('/api/interview/prep', {
                position: pos,
        ...
        // ========== HISTORY ==========
        function saveToHistory(endpoint, body) {
            const key = 'ra_history_' + endpoint.split('/').pop();
            const items = JSON.parse(localStorage.getItem(key) || '[]');
            items.unshift({ time: Date.now(), preview: JSON.stringify(body).slice(0, 60) + '...' });
            localStorage.setItem(key, JSON.stringify(items.slice(0, 5)));
        }
        function loadHistory() {
            // Optional: display history badges per module
        }
    </script>
</body>
</html>
